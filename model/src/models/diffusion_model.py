...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def _safe_print(self, *args, **kwargs):
        """Print that works with or without a Trainer attached."""
        try:
            # Call the original PyTorch Lightning print method
            pl.LightningModule.print(self, *args, **kwargs)
        except RuntimeError:
            # No trainer attached, use regular print
            print(*args, **kwargs)

    def training_step(self, data, i):
        if data.edge_index.numel() == 0:
            self._safe_print("Found a batch with no edges. Skipping.")
            return
    ...
    def on_fit_start(self) -> None:
        ...
        self._safe_print("Size of the input features", self.Xdim, self.Edim, self.ydim)
        # W&B is now initialized via WandbLogger in train.py (DDP-safe)

    def on_train_epoch_start(self) -> None:
        self._safe_print("Starting train epoch...")
    ...
    def on_train_epoch_end(self) -> None:
        ...
        self._safe_print(f"Epoch {self.current_epoch}: {epoch_at_metrics} -- {epoch_bond_metrics}")

        # GPU memory logging (simple one-liner)
        if getattr(self.cfg.train, "log_gpu_memory", False) and torch.cuda.is_available():
            allocated = torch.cuda.memory_allocated() / 1024**3
            reserved = torch.cuda.memory_reserved() / 1024**3
            max_allocated = torch.cuda.max_memory_allocated() / 1024**3
            self._safe_print(f"GPU Memory: {allocated:.2f}GB allocated, {reserved:.2f}GB reserved, {max_allocated:.2f}GB peak")

        # Full memory summary (verbose)
        if getattr(self.cfg.train, "log_memory_summary", False) and torch.cuda.is_available():
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self._safe_print(f"Epoch {self.current_epoch}: Val NLL {metrics[0] :.2f} -- Val Atom type KL {metrics[1] :.2f} -- ",
        ...
        self._safe_print('Val loss: %.4f \t Best val loss:  %.4f\n' % (val_nll, self.best_val_nll))
    ...
    def on_test_epoch_start(self) -> None:
        self._safe_print("Starting test...")
        self.test_nll.reset()
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self._safe_print(f"Epoch {self.current_epoch}: Test NLL {metrics[0] :.2f} -- Test Atom type KL {metrics[1] :.2f} -- ",
        ...
        self._safe_print(f'Test loss: {test_nll :.4f}')
        ...
        if self.local_rank == 0:
            ...
            id = 0
            while samples_left_to_generate > 0:
                self._safe_print(f'Samples left to generate: {samples_left_to_generate}/'
    ...
    @torch.no_grad()
    def forward_noising_at_step(self, X_0, E_0, y_0, node_mask, t_int):
        """Sample the q-process up to an explicit timestep ``t``.

        We mirror ``apply_noise`` but expose ``t`` so evaluation can corrupt a
        given sample deterministically. All tensors must already be masked with
        padded nodes set to zero.
        """

        batch_size = X_0.size(0)
        if isinstance(t_int, (int, float)):
            t_int = torch.full((batch_size, 1), float(t_int), device=X_0.device)
        else:
            t_int = t_int.to(device=X_0.device, dtype=torch.float32)

        t_int = torch.clamp(t_int, 0.0, float(self.T))
        s_int = torch.clamp(t_int - 1.0, min=0.0)
        ...
        probX = torch.matmul(X_0, Qtb.X)
        probE = torch.matmul(E_0, Qtb.E.unsqueeze(1))

        sampled_t = diffusion_utils.sample_discrete_features(probX=probX, probE=probE, node_mask=node_mask)
        X_t = F.one_hot(sampled_t.X, num_classes=self.Xdim_output).type_as(X_0)
        E_t = F.one_hot(sampled_t.E, num_classes=self.Edim_output).type_as(E_0)
        z_t = utils.PlaceHolder(X=X_t, E=E_t, y=y_0).mask(node_mask)
    ...
    @torch.no_grad()
    def recover_from_step(self, X_t, E_t, y_t, node_mask, t_int):
        """Run the learned reverse process from timestep ``t`` back to zero."""

        if isinstance(t_int, (int, float)):
            t_vals = torch.full((X_t.size(0), 1), float(t_int), device=X_t.device)
        else:
            t_vals = t_int.to(device=X_t.device, dtype=torch.float32)
        t_vals = torch.clamp(t_vals, 0.0, float(self.T))
        max_t = int(torch.max(t_vals).item())

        X_cur, E_cur, y_cur = X_t, E_t, y_t

        for step in range(max_t, 0, -1):
            s_array = torch.full((X_cur.size(0), 1), float(step - 1), device=X_cur.device)
            t_array = torch.full((X_cur.size(0), 1), float(step), device=X_cur.device)
            s_norm = s_array / self.T
            t_norm = t_array / self.T

            sampled_s, _ = self.sample_p_zs_given_zt(s_norm, t_norm, X_cur, E_cur, y_cur, node_mask)
            sampled_s = sampled_s.mask(node_mask)
            X_cur, E_cur, y_cur = sampled_s.X, sampled_s.E, sampled_s.y

        final = utils.PlaceHolder(X=X_cur, E=E_cur, y=y_cur).mask(node_mask, collapse=True)
        return final.X, final.E, final.y
